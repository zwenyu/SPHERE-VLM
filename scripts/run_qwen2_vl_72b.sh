...
for file in ${single_skill_json[@]}; do
    echo ${file}
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python main.py --model_name ${model_name} --annotations_json single_skill/${file} --save_predictions
done

for file in ${combine_2_skill_json[@]}; do
    echo ${file}
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python main.py --model_name ${model_name} --annotations_json combine_2_skill/${file} --save_predictions
done

for file in ${reasoning_json[@]}; do
    echo ${file}
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python main.py --model_name ${model_name} --annotations_json reasoning/${file} --save_predictions
done